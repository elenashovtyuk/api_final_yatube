...
class AuthorOrReadOnly(permissions.BasePermission):
    ...
    # здесь мы проверяем -
    # если метод запроса безопасный или аутентифицированный пользователь из запроса
    # является автором(поста или коммента), то доступ к объекту разрешен
    def has_object_permission(self, request, view, obj):
        return obj.owner == request.user
    # после создания кастомного пермишн, добавляем его в аттрибут permission_classes
